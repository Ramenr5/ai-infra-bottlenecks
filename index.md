...
- [[Thesis stress-tests]] — **the adjudication engine.** Rubric + dated ledger for testing whether a new claim/theory holds up against the evidence base (feasibility score, contradiction check, thesis impact, falsifier). Inaugural test: Michael Burry's depreciation argument → **Contested** (mechanism real, core premise rebutted by rising GPU rental prices). Run on-demand + auto-linted by the Regime Monitor.
...
- [[LeaderDrive]] — **China's #1 harmonic-reducer maker** (~30-40% share); Goldman's **top-ranked** humanoid component; spans all 3 actuator types; content-per-robot play **but ~100× sales** (high-beta call option) (ticker: 688017.SS · layer: application)
- [[Schaeffler]] — **Western Tier-1 toll**: integrated humanoid actuator + planetary roller screw + bearings (CES 2026 all-in-one); growth option on a cyclical auto/industrial core (ticker: SHA.DE · layer: application)
- [[Novanta]] — **force/torque-sensor toll** (owns ATI); 6-axis sensing = dexterity-critical; Tier-1, US-listed (ticker: NOVT · layer: application)
- [[Inovance]] — **"Chinese ABB"**: servo/motor/PLC + humanoid dexterous-hand; 5M+ joint motors '25, outgrowing ABB/Siemens; Tier-2 (ticker: 300124.SZ · layer: application)
- [[Sanhua]] — **Tesla Optimus actuator-assembly winner** (~$685M order, ~180k units); highest adoption-certainty but thinner-rent (Tier-2) (ticker: 002050.SZ · layer: application)
- [[Sunny Optical]] — diversified **optics/vision toll** (HK-listed); robot vision modules; lower-beta vs pure LiDAR (ticker: 2382.HK · layer: application)
...
- ⭐ [[NVIDIA Computex 2026 Keynote - ServeTheHome Live (2026)]] — **eyewitness slide-by-slide liveblog, image-processed.** Verifies the anchor off the actual stage slides: full Vera CPU spec sheet; the "Compute is Revenue" 4-lever framework (tokens/watt · TTFT · MTBI · useful life); DSX AI-factory platform (MaxLPS/Flex) + named partner ecosystem; Vera adopters (OpenAI/Anthropic/SpaceX); Cosmos Coalition. Resolves the Vera "1.8× (slide) vs 1.5× (blog)" question.
